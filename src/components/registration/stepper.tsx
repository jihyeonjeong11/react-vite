...
const Stepper = ({ steps, currentStep }: { steps: string[]; currentStep: number }) => {
    ...
    const stepsRef = useRef<StepProps[] | undefined>();

    const updateStep = (stepNumber: number, steps: StepProps[]) => {
            ...
            >
                <div className="relative flex flex-col items-center text-teal-600">
                    <div
                        className={`rounded-full transition duration-500 ease-in-out border-2 border-gray-300 h-12 w-12 flex items-center justify-center py-3  ${
                            step.selected ? "bg-brand-100 text-white font-bold border border-brand-100 " : ""
                        }`}
                    >
                ...
                <div
                    className={`flex-auto border-t-2 transition duration-500 ease-in-out  ${
                        step.completed ? "bg-brand-100" : "border-gray-300 "
                    }  `}
                ></div>
            </div>
        );
